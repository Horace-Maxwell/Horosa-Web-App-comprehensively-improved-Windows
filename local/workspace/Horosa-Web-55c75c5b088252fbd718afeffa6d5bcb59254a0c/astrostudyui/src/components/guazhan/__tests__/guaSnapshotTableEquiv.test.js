...
// [Q-110 裁决 A 2026-09-18] 冻结基线里「流派：通用」随 default 档标签改「通用（卜筮正宗口径）」,其余逐字节不动。
...
// [六爻补齐 A8] 快照尾部有意追加 [断诀命中]/[占类断语] 两段(builder 恒产,导出层按设置控)。
// 本套件守「既有段字节不变」——比对前剥掉尾部新段;新段自身由 liuyaoDuanJue.test 抵引擎守。
const stripNewSections = (t)=>`${t}`.split('\n\n[断诀命中]')[0];

// [Q-448/T-411] 本套件证明的是**表化(行式→GFM 表)信息无损**,不是「快照内容永久冻结」——
// BASELINE 仍严禁重生成(那会把表化证明本身作废),但此后有意新增的内容须在比对前归一掉,
// 与上面 stripNewSections 同一手法。本轮新增两项(各由 guaSnapshot.test 的专项用例守):
//   ① [六爻与动爻] 增 伏神卦 / 综卦 / 错卦 三块逐爻装卦(页面「关联卦」区早已五卦俱全);
//   ② [断卦结构] 逐爻表 状态列 增 岁破 / 日破(页面装卦表状态列早有);
//   ③ [断卦结构] 增「次用神」「取用说明」两行(页面用神卡早有)。
const NEW_YAO_BLOCKS = /^(伏神卦\(本宫首卦\)|综卦|错卦)逐爻（初→上）：$/;
const NEW_YONGSHEN_LINES = /^(次用神|取用说明)：/;
function stripNewRelatedBlocks(t){
	const src = `${t}`.split('\n');
	const out = [];
	let skipping = false;
	src.forEach((l)=>{
		if(NEW_YONGSHEN_LINES.test(l)){ return; }
		if(NEW_YAO_BLOCKS.test(l)){ skipping = true; return; }
		if(skipping){
			if(/^第\d+爻：/.test(l)){ return; }
			skipping = false;
		}
		out.push(l);
	});
...
// 状态列剔除本轮新增的 岁破/日破 两 token(其余 token 与顺序不动)。
const STRIP_PO = (stat)=>`${stat || ''}`.split(',').filter((x)=>x && x !== '岁破' && x !== '日破').join(',');
...
function rebuildYao(cells){
	const [yao, liu, zhi, wx, lq, sy, ws, stat0, fu, sha] = cells.map(de);
	const stat = STRIP_PO(stat0);   // [Q-448/T-411] 新增的岁破/日破不参与 v1 逐字节比对
...
describe('六爻 [断卦结构] 表化等价', ()=>{
	...
	it('逆变换:新快照表反拼 v1 旧行,逐字节 == 冻结基线', ()=>{
		expect(inverse(stripNewRelatedBlocks(stripNewSections(txt)))).toBe(BASELINE);
	});
	...
	it('逐爻表元组集合 == 旧行独立解析元组集合(零信息丢失)', ()=>{
		...
		// [Q-448/T-411] 状态列的岁破/日破为本轮新增事实,归一后再与 v1 元组集比对。
		const normed = yaoT.rows.map((r)=>r.map((c, i)=>(i === 7 ? STRIP_PO(de(c)) : c)));
		expect(tset(normed)).toEqual(tset(oldYaoTuples()));
	});
	...
	it('段头序零变更(表化只动段内)', ()=>{
		...
		expect(heads(stripNewSections(txt))).toEqual(heads(BASELINE));
	});
