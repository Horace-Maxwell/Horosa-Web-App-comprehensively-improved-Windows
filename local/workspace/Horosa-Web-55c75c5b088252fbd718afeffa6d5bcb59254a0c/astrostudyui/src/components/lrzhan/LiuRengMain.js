...
class LiuRengMain extends Component{
	...
	async requestGods(fields, chartSnapshot){
		...
		const data = await memoizedJsonRequest(`${Constants.ServerRoot}/liureng/gods`, params, {}, {
			namespace: 'liureng.gods',
			maxSize: 128,
		});
	...
	async requestRunYear(fields){
		...
		const data = await memoizedJsonRequest(`${Constants.ServerRoot}/liureng/runyear`, params, {}, {
			namespace: 'liureng.runyear',
			maxSize: 128,
		});
		const result = data[Constants.ResultKey]
				...
				<Row gutter={6} style={{ height: '100%' }}>
					...
					<Col span={8} style={{ height: '100%' }}>
						<div
							...
							style={{ height: rightPanelHeight, maxHeight: panelLimit, overflow: 'hidden', paddingRight: 2 }}
						>
								...
								<Card size="small" className={styles.judgeCard} bodyStyle={{ padding: '10px 12px' }}>
									...
									>
										<TabPane tab="大格" key="dage">
											<div className={styles.judgePaneScroll}>
