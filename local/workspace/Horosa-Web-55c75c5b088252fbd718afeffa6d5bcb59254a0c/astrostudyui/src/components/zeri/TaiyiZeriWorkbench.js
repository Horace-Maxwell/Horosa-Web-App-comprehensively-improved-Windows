// [Z3·太乙择日] 择时工作台:三视图逐字对齐奇门/黄历——左列:①时间范围·地点·参数(五参:
// (太乙无本命上下文条件——太乙候选时刻盘无本命关系语义)
	...
	return (
		<div key={`l${counter.i}`} style={{ padding: '5px 0 5px 0', marginLeft: depth ? 14 : 0, borderBottom: '1px dashed rgba(148,163,184,.16)' }}>
			<div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', fontSize: 12 }}>
				...
				<span>{ui ? taiyiLeafSummary(ui) : ((TAIYI_CONDITION_TYPES[node.type] || {}).label || node.type)}</span>
			</div>
...
const CATEGORY_ORDER = (()=>{
	const seen = ['局式', '落宫', '格局', '算数', '大将', '十六宫', '门户'];
	Object.keys(TAIYI_CONDITION_TYPES).forEach((k)=>{
		const c = TAIYI_CONDITION_TYPES[k].category;
...
export default function TaiyiZeriWorkbench({
	...
	const [draftParams, setDraftParams] = useState(() => newTaiyiLeaf('geju_kind').params);
	...
	const resetDraft = (type) => {
		setDraftType(type);
		setDraftParams(newTaiyiLeaf(type).params);
	...
	const doAddGroup = () => {
		onTreeChange(mapAt(tree, appendTargetPath, (g) => ({ ...g, children: [...g.children, newTaiyiGroup('any')] })));
	};
	const doRemove = (path) => {
	...
	const __schemeIssues = (rec && rec.tree) ? auditTreeAgainstRegistry(rec.tree, TAIYI_CONDITION_TYPES) : [];
	...
	const schemes = taiyiZeriSchemeStore.listSchemes();
	const history = taiyiZeriSchemeStore.listHistory();
	const schemeMenu = (
		<Menu>
				...
				<Menu.Item key={s.id}>
					<span style={{ display: 'inline-flex', gap: 8, alignItems: 'center' }}>
						...
						<a style={{ color: '#e5484d', fontSize: 11 }}
							onClick={(e) => { e.preventDefault(); e.stopPropagation(); taiyiZeriSchemeStore.deleteScheme(s.id); setSchemeTick(schemeTick + 1); }}>删</a>
					</span>
				</Menu.Item>
			...
			<div key={path.join('.')}
				...
				<div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
					...
					{isGroup ? (
						...
						<span style={{ fontSize: 12 }}>{taiyiLeafSummary({ ...node, negate: false })}</span>
					)}
					<span style={{ flex: 1 }} />
		...
		<div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 440px', gap: 12, height: 'clamp(560px, calc(100vh - 220px), 900px)' }}>
			...
			<div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, border: '1px solid rgba(148,163,184,.25)', borderRadius: 8 }}>
				...
				<div style={{ flex: 1, overflowY: 'auto', padding: 10 }}>
						...
						<XQSelect size="small" style={{ minWidth: 150 }} value={draftType}
							...
							{CATEGORY_ORDER.map((cat) => (
								<OptGroup label={cat} key={cat}>
									{Object.entries(TAIYI_CONDITION_TYPES).filter(([, spec]) => spec.category === cat).map(([key, spec]) => (
					...
					<ConditionParamsForm types={TAIYI_CONDITION_TYPES} type={draftType} params={draftParams} onChange={setDraftParams} />
					<div style={{
				...
				<div style={{ flex: 1, minHeight: 0, border: '1px solid rgba(148,163,184,.25)', borderRadius: 8, display: 'flex', flexDirection: 'column' }}>
					...
					<div style={{ padding: '6px 10px', borderTop: '1px solid rgba(148,163,184,.2)', display: 'flex', gap: 8, alignItems: 'center' }}>
						...
						<XQButton size="small" disabled={!schemeName.trim()} onClick={() => {
							const r = taiyiZeriSchemeStore.saveScheme(schemeName, { cfg, geo, options }, tree);
		...
		<div style={{ height: 'clamp(560px, calc(100vh - 220px), 900px)', display: 'flex', flexDirection: 'column' }}>
			...
			<div style={{ flex: 1, minHeight: 0, overflowY: 'auto', border: '1px solid rgba(148,163,184,.25)', borderRadius: 8 }}>
				<div style={{ display: 'flex', fontSize: 12, opacity: 0.6, padding: '8px 12px', gap: 10, borderBottom: '1px solid rgba(148,163,184,.2)', position: 'sticky', top: 0, background: 'var(--horosa-astro-panel, #fff)', zIndex: 1 }}>
					...
					<span style={{ width: 170, textAlign: 'center' }}>局·太乙</span>
					...
					<div key={i} style={{ borderBottom: '1px dashed rgba(148,163,184,.18)' }}>
						<div style={{ display: 'flex', gap: 10, padding: '9px 12px', alignItems: 'center' }}>
							...
							<span style={{ width: 150, textAlign: 'center', fontSize: 12 }}>
								<ZeriRowBadge text={row.juText ? `${row.juText}·${row.taiyiText || ''}` : ''} />
							</span>
	...
	const onImportFile = (e) => {
		...
		reader.onload = () => {
			const r = taiyiZeriSchemeStore.importSchemes(reader.result);
		...
		<div style={{ height: 'clamp(560px, calc(100vh - 220px), 900px)', display: 'flex', flexDirection: 'column' }}>
			<div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
				...
				<XQButton size="small" disabled={!schemes.length}
					onClick={() => downloadJson(taiyiZeriSchemeStore.exportSchemes(), `太乙择日方案_${new Date().toISOString().slice(0, 10)}.json`)}>
					导出全部(JSON)
				</XQButton>
			</div>
				...
				{schemes.map((s) => (
					<div key={s.id} style={{ display: 'flex', gap: 10, padding: '8px 12px', alignItems: 'center', borderBottom: '1px dashed rgba(148,163,184,.18)' }}>
						{renameId === s.id ? (
							<span style={{ flex: 1, display: 'inline-flex', gap: 6 }}>
								...
								<XQButton size="small" type="primary" onClick={() => {
									const r = taiyiZeriSchemeStore.renameScheme(s.id, renameText);
						...
						<span style={{ width: 250, textAlign: 'right', display: 'inline-flex', gap: 6, justifyContent: 'flex-end' }}>
							...
							<XQButton size="small" onClick={() => downloadJson(taiyiZeriSchemeStore.exportSchemes([s.id]), `太乙择日方案_${s.name}.json`)}>导出</XQButton>
							<XQButton size="small" danger onClick={() => { taiyiZeriSchemeStore.deleteScheme(s.id); setSchemeTick(schemeTick + 1); }}>删除</XQButton>
						</span>
					</div>
				))}
			</div>
