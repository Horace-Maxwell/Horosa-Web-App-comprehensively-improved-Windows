// [Z5·六壬择日] 择时工作台(紫微版派生)。左列:①时间范围·地点+参数区 6 键(月将两档/
// 贵人体系五流派/阴阳系/时基/换日/晚子时——逐键判别力 liurengZeriEngine 金标证;昼夜=
// 日出方程自动判非档位)+用事人本命区(本命支+行年支,生辰解析)。②构造条件 ③连接门
// ④动作排+「择时」。右列:已选条件链+方案排。结果表「课·三传」列。
...
import {
	LIURENG_CONDITION_TYPES, newLiurengLeaf, newLiurengGroup, liurengLeafSummary,
} from '../../divination/zeri/liurengZeriConditionTypes';
	...
	return (
		<div key={`l${counter.i}`} style={{ padding: '5px 0 5px 0', marginLeft: depth ? 14 : 0, borderBottom: '1px dashed rgba(148,163,184,.16)' }}>
			<div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', fontSize: 12 }}>
				...
				<span>{ui ? liurengLeafSummary(ui) : ((LIURENG_CONDITION_TYPES[node.type] || {}).label || node.type)}</span>
			</div>
...
const CATEGORY_ORDER = (()=>{
	const seen = ['课体', '三传', '发用', '盘式', '四课', '日时', '神煞', '本命'];
	Object.keys(LIURENG_CONDITION_TYPES).forEach((k)=>{
		const c = LIURENG_CONDITION_TYPES[k].category;
...
export default function LiurengZeriWorkbench({
	...
	const [draftParams, setDraftParams] = useState(() => newLiurengLeaf('ke_name').params);
	...
	const resetDraft = (type) => {
		setDraftType(type);
		setDraftParams(newLiurengLeaf(type).params);
	...
	const doAddGroup = () => {
		onTreeChange(mapAt(tree, appendTargetPath, (g) => ({ ...g, children: [...g.children, newLiurengGroup('any')] })));
	};
	const doRemove = (path) => {
	...
	const __schemeIssues = (rec && rec.tree) ? auditTreeAgainstRegistry(rec.tree, LIURENG_CONDITION_TYPES) : [];
	...
	const schemes = liurengZeriSchemeStore.listSchemes();
	const history = liurengZeriSchemeStore.listHistory();
	const schemeMenu = (
		<Menu>
				...
				<Menu.Item key={s.id}>
					<span style={{ display: 'inline-flex', gap: 8, alignItems: 'center' }}>
						...
						<a style={{ color: '#e5484d', fontSize: 11 }}
							onClick={(e) => { e.preventDefault(); e.stopPropagation(); liurengZeriSchemeStore.deleteScheme(s.id); setSchemeTick(schemeTick + 1); }}>删</a>
					</span>
				</Menu.Item>
			...
			<div key={path.join('.')}
				...
				<div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
					...
					{isGroup ? (
						...
						<span style={{ fontSize: 12 }}>{liurengLeafSummary({ ...node, negate: false })}</span>
					)}
					<span style={{ flex: 1 }} />
			...
			<div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, border: '1px solid rgba(148,163,184,.25)', borderRadius: 8 }}>
				<div style={{ padding: 10, borderBottom: '1px solid rgba(148,163,184,.2)' }}>
					...
					<div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', padding: '6px 8px', borderRadius: 8, background: 'rgba(212,175,55,.05)', border: '1px dashed rgba(212,175,55,.4)' }}>
						<span style={{ fontSize: 11, opacity: 0.7 }}>用事人本命(本命·行年)</span>
				...
				<div style={{ flex: 1, overflowY: 'auto', padding: 10 }}>
						...
						<XQSelect size="small" style={{ minWidth: 150 }} value={draftType}
							...
							{CATEGORY_ORDER.map((cat) => (
								<OptGroup label={cat} key={cat}>
									{Object.entries(LIURENG_CONDITION_TYPES).filter(([, spec]) => spec.category === cat).map(([key, spec]) => (
					...
					<ConditionParamsForm types={LIURENG_CONDITION_TYPES} type={draftType} params={draftParams} onChange={setDraftParams} />
					<div style={{
		...
		<div style={{ height: 'clamp(560px, calc(100vh - 220px), 900px)', display: 'flex', flexDirection: 'column' }}>
			...
			<div style={{ flex: 1, minHeight: 0, overflowY: 'auto', border: '1px solid rgba(148,163,184,.25)', borderRadius: 8 }}>
				<div style={{ display: 'flex', fontSize: 12, opacity: 0.6, padding: '8px 12px', gap: 10, borderBottom: '1px solid rgba(148,163,184,.2)', position: 'sticky', top: 0, background: 'var(--horosa-astro-panel, #fff)', zIndex: 1 }}>
					...
					<span style={{ width: 150, textAlign: 'center' }}>课·三传</span>
					...
					<div key={i} style={{ borderBottom: '1px dashed rgba(148,163,184,.18)' }}>
						<div style={{ display: 'flex', gap: 10, padding: '9px 12px', alignItems: 'center' }}>
							...
							<span style={{ width: 150, textAlign: 'center', fontSize: 12 }}>
								<ZeriRowBadge text={row.chuanText ? `${row.keText || '—'}·${row.chuanText}` : (row.keText || '—')} />
							</span>
	...
	const onImportFile = (e) => {
		...
		reader.onload = () => {
			const r = liurengZeriSchemeStore.importSchemes(reader.result);
		...
		<div style={{ height: 'clamp(560px, calc(100vh - 220px), 900px)', display: 'flex', flexDirection: 'column' }}>
			<div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
				...
				<XQButton size="small" disabled={!schemes.length}
					onClick={() => downloadJson(liurengZeriSchemeStore.exportSchemes(), `六壬择日方案_${new Date().toISOString().slice(0, 10)}.json`)}>
					导出全部(JSON)
				</XQButton>
			</div>
				...
				{schemes.map((s) => (
					<div key={s.id} style={{ display: 'flex', gap: 10, padding: '8px 12px', alignItems: 'center', borderBottom: '1px dashed rgba(148,163,184,.18)' }}>
						{renameId === s.id ? (
							<span style={{ flex: 1, display: 'inline-flex', gap: 6 }}>
								...
								<XQButton size="small" type="primary" onClick={() => {
									const r = liurengZeriSchemeStore.renameScheme(s.id, renameText);
						...
						<span style={{ width: 250, textAlign: 'right', display: 'inline-flex', gap: 6, justifyContent: 'flex-end' }}>
							...
							<XQButton size="small" onClick={() => downloadJson(liurengZeriSchemeStore.exportSchemes([s.id]), `六壬择日方案_${s.name}.json`)}>导出</XQButton>
							<XQButton size="small" danger onClick={() => { liurengZeriSchemeStore.deleteScheme(s.id); setSchemeTick(schemeTick + 1); }}>删除</XQButton>
						</span>
					</div>
				))}
			</div>
