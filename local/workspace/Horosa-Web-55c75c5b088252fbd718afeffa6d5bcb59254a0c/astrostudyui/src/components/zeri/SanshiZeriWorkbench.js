// [Z6·三式择日] 择时工作台(六壬版派生)。左列:①时间范围·地点+**三段参数区**(六壬 3 键/
// 奇门 10 键[QimenZeriWorkbench PARAM_FIELDS 同子集,「综合」等后端专属模式不入]/太乙 1 键
// +共享日界 2 键+时基,merged 平铺经 splitSanshiOptions 单源拆三家)+用事人本命区(六壬家用)。
...
import {
	PAIPAN_OPTIONS, QIJU_METHOD_OPTIONS, SCHOOL_OPTIONS, ZHISHI_OPTIONS,
	KONG_MODE_OPTIONS, MA_MODE_OPTIONS, TIME_ALG_OPTIONS, DAY_SWITCH_OPTIONS,
...
import {
	SANSHI_CONDITION_TYPES, newSanshiLeaf, newSanshiGroup, sanshiLeafSummary,
	...
	return (
		<div key={`l${counter.i}`} style={{ padding: '5px 0 5px 0', marginLeft: depth ? 14 : 0, borderBottom: '1px dashed rgba(148,163,184,.16)' }}>
			<div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', fontSize: 12 }}>
				...
				<span>{ui ? sanshiLeafSummary(ui) : ((SANSHI_CONDITION_TYPES[node.type] || {}).label || node.type)}</span>
			</div>
...
// 类别序动态收集(程序化合并:「六壬·课体」「奇门·格局」「太乙·局式」…按家序+首现序)。
const CATEGORY_ORDER = (()=>{
	const seen = [];
	Object.keys(SANSHI_CONDITION_TYPES).forEach((k)=>{
		const c = SANSHI_CONDITION_TYPES[k].category;
...
export default function SanshiZeriWorkbench({
	...
	const [draftParams, setDraftParams] = useState(() => newSanshiLeaf('lr_ke_name').params);
	...
	const resetDraft = (type) => {
		setDraftType(type);
		setDraftParams(newSanshiLeaf(type).params);
	...
	const doAddGroup = () => {
		onTreeChange(mapAt(tree, appendTargetPath, (g) => ({ ...g, children: [...g.children, newSanshiGroup('any')] })));
	};
	const doRemove = (path) => {
	...
	const __schemeIssues = (rec && rec.tree) ? auditTreeAgainstRegistry(rec.tree, SANSHI_CONDITION_TYPES) : [];
	...
	const schemes = sanshiZeriSchemeStore.listSchemes();
	const history = sanshiZeriSchemeStore.listHistory();
	const schemeMenu = (
		<Menu>
				...
				<Menu.Item key={s.id}>
					<span style={{ display: 'inline-flex', gap: 8, alignItems: 'center' }}>
						...
						<a style={{ color: '#e5484d', fontSize: 11 }}
							onClick={(e) => { e.preventDefault(); e.stopPropagation(); sanshiZeriSchemeStore.deleteScheme(s.id); setSchemeTick(schemeTick + 1); }}>删</a>
					</span>
				</Menu.Item>
			...
			<div key={path.join('.')}
				...
				<div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
					...
					{isGroup ? (
						...
						<span style={{ fontSize: 12 }}>{sanshiLeafSummary({ ...node, negate: false })}</span>
					)}
					<span style={{ flex: 1 }} />
			...
			<div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, border: '1px solid rgba(148,163,184,.25)', borderRadius: 8 }}>
				<div style={{ padding: 10, borderBottom: '1px solid rgba(148,163,184,.2)' }}>
					...
					<div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', padding: '6px 8px', borderRadius: 8, background: 'rgba(212,175,55,.05)', border: '1px dashed rgba(212,175,55,.4)' }}>
						<span style={{ fontSize: 11, opacity: 0.7 }}>用事人本命(六壬家条件用)</span>
				...
				<div style={{ flex: 1, overflowY: 'auto', padding: 10 }}>
						...
						<XQSelect size="small" style={{ minWidth: 150 }} value={draftType}
							...
							{CATEGORY_ORDER.map((cat) => (
								<OptGroup label={cat} key={cat}>
									{Object.entries(SANSHI_CONDITION_TYPES).filter(([, spec]) => spec.category === cat).map(([key, spec]) => (
					...
					<ConditionParamsForm types={SANSHI_CONDITION_TYPES} type={draftType} params={draftParams} onChange={setDraftParams} />
					<div style={{
		...
		<div style={{ height: 'clamp(560px, calc(100vh - 220px), 900px)', display: 'flex', flexDirection: 'column' }}>
			...
			<div style={{ flex: 1, minHeight: 0, overflowY: 'auto', border: '1px solid rgba(148,163,184,.25)', borderRadius: 8 }}>
				<div style={{ display: 'flex', fontSize: 12, opacity: 0.6, padding: '8px 12px', gap: 10, borderBottom: '1px solid rgba(148,163,184,.2)', position: 'sticky', top: 0, background: 'var(--horosa-astro-panel, #fff)', zIndex: 1 }}>
					...
					<span style={{ width: 150, textAlign: 'center' }}>三家(课·局·局)</span>
					...
					<div key={i} style={{ borderBottom: '1px dashed rgba(148,163,184,.18)' }}>
						<div style={{ display: 'flex', gap: 10, padding: '9px 12px', alignItems: 'center' }}>
							...
							<span style={{ width: 150, textAlign: 'center', fontSize: 12 }}>
								<ZeriRowBadge text={row.sanshiText} />
							</span>
	...
	const onImportFile = (e) => {
		...
		reader.onload = () => {
			const r = sanshiZeriSchemeStore.importSchemes(reader.result);
		...
		<div style={{ height: 'clamp(560px, calc(100vh - 220px), 900px)', display: 'flex', flexDirection: 'column' }}>
			<div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
				...
				<XQButton size="small" disabled={!schemes.length}
					onClick={() => downloadJson(sanshiZeriSchemeStore.exportSchemes(), `三式择日方案_${new Date().toISOString().slice(0, 10)}.json`)}>
					导出全部(JSON)
				</XQButton>
			</div>
				...
				{schemes.map((s) => (
					<div key={s.id} style={{ display: 'flex', gap: 10, padding: '8px 12px', alignItems: 'center', borderBottom: '1px dashed rgba(148,163,184,.18)' }}>
						{renameId === s.id ? (
							<span style={{ flex: 1, display: 'inline-flex', gap: 6 }}>
								...
								<XQButton size="small" type="primary" onClick={() => {
									const r = sanshiZeriSchemeStore.renameScheme(s.id, renameText);
						...
						<span style={{ width: 250, textAlign: 'right', display: 'inline-flex', gap: 6, justifyContent: 'flex-end' }}>
							...
							<XQButton size="small" onClick={() => downloadJson(sanshiZeriSchemeStore.exportSchemes([s.id]), `三式择日方案_${s.name}.json`)}>导出</XQButton>
							<XQButton size="small" danger onClick={() => { sanshiZeriSchemeStore.deleteScheme(s.id); setSchemeTick(schemeTick + 1); }}>删除</XQButton>
						</span>
					</div>
				))}
			</div>
