// [Z8·印度择日] 择时工作台(七政版派生;远端 Muhurta 扫描形)。参数:岁差制(lahiri/raman/kp)
// /罗计交点(平/真)/本命月宿·月座(0=未设;设后 Tara/Chandra 本命组条件生效,印度页盘面可查
// 本命月位)。结果分钟级区间(/indiaelectionscan,93 天限)。
...
import {
	INDIA_CONDITION_TYPES, newIndiaLeaf, newIndiaGroup, indiaLeafSummary, IN_NAK27, IN_SIGNS,
} from '../../divination/zeri/indiaZeriConditionTypes';
	...
	return (
		<div key={`l${counter.i}`} style={{ padding: '5px 0 5px 0', marginLeft: depth ? 14 : 0, borderBottom: '1px dashed rgba(148,163,184,.16)' }}>
			<div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', fontSize: 12 }}>
				...
				<span>{ui ? indiaLeafSummary(ui) : ((INDIA_CONDITION_TYPES[node.type] || {}).label || node.type)}</span>
			</div>
...
const CATEGORY_ORDER = (()=>{
	const seen = ['五肢', '上升', '曜位', '日段', '本命'];
	Object.keys(INDIA_CONDITION_TYPES).forEach((k)=>{
		const c = INDIA_CONDITION_TYPES[k].category;
		if(c && !seen.includes(c)){ seen.push(c); }
	});
	return seen;
})();

export default function IndiaZeriWorkbench({
	...
	const [draftType, setDraftType] = useState('tithi');	// 🔴 初值必须是本注册表键(曾抄太乙 geju_kind=首开未知条件,审查实抓)
	const [draftParams, setDraftParams] = useState(() => newIndiaLeaf('tithi').params);
	...
	const resetDraft = (type) => {
		setDraftType(type);
		setDraftParams(newIndiaLeaf(type).params);
	...
	const draftSpec = INDIA_CONDITION_TYPES[draftType] || {};
	...
	const doAddGroup = () => {
		onTreeChange(mapAt(tree, appendTargetPath, (g) => ({ ...g, children: [...g.children, newIndiaGroup('any')] })));
	};
	const doRemove = (path) => {
	...
	const __schemeIssues = (rec && rec.tree) ? auditTreeAgainstRegistry(rec.tree, INDIA_CONDITION_TYPES) : [];
	...
	const schemes = indiaZeriSchemeStore.listSchemes();
	const history = indiaZeriSchemeStore.listHistory();
	const schemeMenu = (
		<Menu>
				...
				<Menu.Item key={s.id}>
					<span style={{ display: 'inline-flex', gap: 8, alignItems: 'center' }}>
						...
						<a style={{ color: '#e5484d', fontSize: 11 }}
							onClick={(e) => { e.preventDefault(); e.stopPropagation(); indiaZeriSchemeStore.deleteScheme(s.id); setSchemeTick(schemeTick + 1); }}>删</a>
					</span>
				</Menu.Item>
			...
			<div key={path.join('.')}
				...
				<div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
					...
					{isGroup ? (
						...
						<span style={{ fontSize: 12 }}>{indiaLeafSummary({ ...node, negate: false })}</span>
					)}
					<span style={{ flex: 1 }} />
			...
			<div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, border: '1px solid rgba(148,163,184,.25)', borderRadius: 8 }}>
				<div style={{ padding: 10, borderBottom: '1px solid rgba(148,163,184,.2)' }}>
					...
					<div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 8 }}>
						...
						{/* 口径+本命(判别力 pytest 金标证;本命两键 0=未设,设后本命组条件生效) */}
						{[
							{ key: 'ayanamsa', label: '岁差制', dv: 'lahiri', options: [{ value: 'lahiri', label: 'Lahiri(默认)' }, { value: 'raman', label: 'Raman' }, { value: 'krishnamurti', label: 'KP' }] },
							{ key: 'nodeType', label: '罗计交点', dv: 'mean', options: [{ value: 'mean', label: '平交点' }, { value: 'true', label: '真交点' }] },
							{ key: 'natalMoonNak', label: '本命月宿', dv: 0, options: [{ value: 0, label: '未设' }, ...IN_NAK27.map((n, i) => ({ value: i + 1, label: `${i + 1}.${n}` }))] },
							{ key: 'natalMoonSign', label: '本命月座', dv: 0, options: [{ value: 0, label: '未设' }, ...IN_SIGNS.map((n, i) => ({ value: i + 1, label: n }))] },
						].map((f) => (
				...
				<div style={{ flex: 1, overflowY: 'auto', padding: 10 }}>
						...
						<XQSelect size="small" style={{ minWidth: 150 }} value={draftType}
							...
							{CATEGORY_ORDER.map((cat) => (
								<OptGroup label={cat} key={cat}>
									{Object.entries(INDIA_CONDITION_TYPES).filter(([, spec]) => spec.category === cat).map(([key, spec]) => (
					...
					<ConditionParamsForm types={INDIA_CONDITION_TYPES} type={draftType} params={draftParams} onChange={setDraftParams} />
					<div style={{
				...
				<div style={{ flex: 1, minHeight: 0, border: '1px solid rgba(148,163,184,.25)', borderRadius: 8, display: 'flex', flexDirection: 'column' }}>
					...
					<div style={{ padding: '6px 10px', borderTop: '1px solid rgba(148,163,184,.2)', display: 'flex', gap: 8, alignItems: 'center' }}>
						...
						<XQButton size="small" disabled={!schemeName.trim()} onClick={() => {
							const r = indiaZeriSchemeStore.saveScheme(schemeName, { cfg, geo, options }, tree);
	...
	const resultView = (
		<div style={{ height: 'clamp(560px, calc(100vh - 220px), 900px)', display: 'flex', flexDirection: 'column' }}>
			<div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
				...
				{scanning && progress ? (
					<span style={{ fontSize: 12, opacity: 0.75 }}>扫描中 第{progress.done}/{progress.total}段 · 已命中 {progress.hits}</span>
				) : null}
	...
	const onImportFile = (e) => {
		...
		reader.onload = () => {
			const r = indiaZeriSchemeStore.importSchemes(reader.result);
		...
		<div style={{ height: 'clamp(560px, calc(100vh - 220px), 900px)', display: 'flex', flexDirection: 'column' }}>
			<div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
				...
				<XQButton size="small" disabled={!schemes.length}
					onClick={() => downloadJson(indiaZeriSchemeStore.exportSchemes(), `印度择日方案_${new Date().toISOString().slice(0, 10)}.json`)}>
					导出全部(JSON)
				</XQButton>
			</div>
				...
				{schemes.map((s) => (
					<div key={s.id} style={{ display: 'flex', gap: 10, padding: '8px 12px', alignItems: 'center', borderBottom: '1px dashed rgba(148,163,184,.18)' }}>
						{renameId === s.id ? (
							<span style={{ flex: 1, display: 'inline-flex', gap: 6 }}>
								...
								<XQButton size="small" type="primary" onClick={() => {
									const r = indiaZeriSchemeStore.renameScheme(s.id, renameText);
						...
						<span style={{ width: 250, textAlign: 'right', display: 'inline-flex', gap: 6, justifyContent: 'flex-end' }}>
							...
							<XQButton size="small" onClick={() => downloadJson(indiaZeriSchemeStore.exportSchemes([s.id]), `印度择日方案_${s.name}.json`)}>导出</XQButton>
							<XQButton size="small" danger onClick={() => { indiaZeriSchemeStore.deleteScheme(s.id); setSchemeTick(schemeTick + 1); }}>删除</XQButton>
						</span>
					</div>
				))}
			</div>
