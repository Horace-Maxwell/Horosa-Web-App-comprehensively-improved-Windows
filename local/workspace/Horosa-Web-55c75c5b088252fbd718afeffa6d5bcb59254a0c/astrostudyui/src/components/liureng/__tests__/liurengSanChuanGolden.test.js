...
//  ③ 课经锁（#46 纠正）：八专(干支同位,止两课)有克照常以克贼比涉论；无克「不复取遥克」，
//     径入八专（阳日干上神顺数三/阴日第四课上神逆数三,中末皆干上神）。
//     🔴 曾把九法列举序号(遥克4/八专9)误读为判定优先级并按其锁过金标——列举序≠判定序，勿再翻案。
...
describe('大六壬 · 课经锁：八专无克不复取遥克（#46 纠正）', () => {
	// [标签, 月将, 占时, 日干, 日支, 期望课名, 期望三传]
	const BAZHUAN = [
		// #46 实报第一例(甲寅日 2026-04-10 丑时,月将戌):两课无近克,申金虽遥克日干甲,按课经径入八专。
		// 阳日:干上神亥顺数三(亥子丑)发用丑,中末皆干上神亥。旧误判:蒿矢。
		['#46 甲寅日 戌将丑时(阳日顺数三)', '戌', '丑', '甲', '寅', '八专课', ['丑', '亥', '亥']],
		// #46 实报第二例(同日午时):日干甲虽遥克戌土,仍八专。阳日:午顺数三(午未申)发用申。旧误判:弹射。
		['#46 甲寅日 戌将午时(阳日顺数三)', '戌', '午', '甲', '寅', '八专课', ['申', '午', '午']],
		// 古籍成例:己未日辰将午时,卦得八专,三传丑巳巳(阴日:第四课上神卯逆数三 卯寅丑 发用丑,
		// 中末皆干上神巳)。此例卯木亦遥克日干己土,旧序会误发蒿矢——正是判别锚。
		['古籍成例 己未日 辰将午时(阴日逆数三)', '辰', '午', '己', '未', '八专课', ['丑', '巳', '巳']],
	];
	BAZHUAN.forEach(([label, yue, tm, gan, zhi, name, exp]) => {
		test(`${label} → ${name} ${exp.join('')}`, () => {
			const r = makeChart(yue, tm, gan, zhi).getSangCuang();
			expect(r.name).toBe(name);
			expect(r.cuang).toEqual(exp);
		});
	});
	test('八专有克照常以克贼比涉论:甲寅日 巳将子时 → 比用课 子巳戌(不入八专)', () => {
		const r = makeChart('巳', '子', '甲', '寅').getSangCuang();
		expect(r.name).toBe('比用课');
		expect(r.cuang).toEqual(['子', '巳', '戌']);
	});
});
...
// ── #62 课经锁:伏吟初中子卯互刑(刑还初传)末传取冲 ──
// 病史:伏吟末传沿「中传所刑」取,而刑表子↔卯为唯一二环——中传所刑还回初传时旧码照取,
// 丁卯/己卯/辛卯日伏吟排出「卯子卯」(#62 实报丁卯/己卯;辛卯同病未报)。勘正:初中两传
// 恰为子卯互刑(传行杜塞)→ 末传取中传所冲。全域 720 课仅此 3 课变化;课名不动(仍自信课)。
// 🔴 720 对拍 oracle 的伏吟段曾与引擎同盲区(对拍全绿掩盖此病)——用户实报课式必须落独立金标锚。
describe('大六壬 · 课经锁:伏吟子卯互刑末传取冲(#62 勘正)', () => {
	const ZIMAO = [
		// [标签, 月将, 占时, 日干, 日支, 期望课名, 期望三传]
		['#62 丁卯日 未将未时(2026-06-22 未时,夏至月将未)', '未', '未', '丁', '卯', '自信课', ['卯', '子', '午']],
		['#62 己卯日 卯将卯时(2026-11-01 卯时,霜降后月将卯)', '卯', '卯', '己', '卯', '自信课', ['卯', '子', '午']],
		['同病 辛卯日 子将子时(戌土生辛金无克,全六十日第三个也是最后一个命中日)', '子', '子', '辛', '卯', '自信课', ['卯', '子', '午']],
	];
	ZIMAO.forEach(([label, yue, tm, gan, zhi, name, exp]) => {
		test(`${label} → ${name} ${exp.join('')}`, () => {
			const r = makeChart(yue, tm, gan, zhi).getSangCuang();
			expect(r.name).toBe(name);
			expect(r.cuang).toEqual(exp);
	...
	// 反越界锚:守卫只认「初中两传互刑成环」;子卯出现在中末相邻不得改写(用户明令只修子卯环)。
	test('反越界:乙卯日伏吟(有克不虞,中卯末子非初中环)仍 辰卯子', () => {
		const r = makeChart('子', '子', '乙', '卯').getSangCuang();
		expect(r.name).toBe('不虞课');
		expect(r.cuang).toEqual(['辰', '卯', '子']);
	});
	test('反越界:壬子日伏吟(杜传,中子末卯非初中环)仍 亥子卯', () => {
		const r = makeChart('子', '子', '壬', '子').getSangCuang();
		expect(r.name).toBe('杜传课');
		expect(r.cuang).toEqual(['亥', '子', '卯']);
	});
});
