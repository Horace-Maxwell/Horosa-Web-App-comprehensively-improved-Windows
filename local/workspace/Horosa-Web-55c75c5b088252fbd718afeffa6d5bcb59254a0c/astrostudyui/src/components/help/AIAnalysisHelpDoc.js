		...
		return (
			<div style={{ marginTop: 10, borderTop: '1px solid var(--horosa-border, rgba(120,120,120,0.25))', paddingTop: 8 }}>
				...
				<Tabs defaultActiveKey="overview" size="small">
					...
					<TabPane tab="挂载与上下文预算" key="context">
						<div style={body}>
							<p style={{ ...p, color: MUTED }}>「上下文」= 每次发送给模型的全部内容:案例快照 + 技法判读 + 引用资料 / 检索片段 + 系统提示 + 你的问题。为避免过长被截断或超额,系统会实时统计并在超限时自动裁剪。</p>
							<div style={card}><div style={ct}>挂载状态条(常驻)</div>
								{kv('位置', '紧贴在对话输入框上方,始终显示「当前要发什么」')}
								{kv('已挂载', '📊 / 📋 案例名 · 🧮 N 技法 · 📚 N 资料 / 组合 · 「上下文约 N / 上限字」· 右侧「编辑」打开挂载预览抽屉')}
								{kv('未挂载', '显示「未挂载案例 · 选择案例」,点一下即打开抽屉选案例')}
								{kv('字数读数', '「上下文约 N / 上限字」是最近一次发送时实际纳入的字数与上限(约两万字);若有整层被丢会在悬浮提示里标明层数')}</div>
							<div style={card}><div style={ct}>上下文预算与自动裁剪</div>
								{kv('触发', '仅当上下文超出上限时才裁剪;不超限则原样全量发送,状态条与预览卡不出现裁剪标记')}
								{kv('超预算未纳入', '红色标签:该层整层未纳入本次发送。多见于同时挂了很多技法。减少挂载技法或在该层「设置」里精简纳入内容即可')}
								{kv('已裁剪 X→Y 字', '橙色标签:该层被按段落边界公平裁剪后纳入,X 为原字数、Y 为保留字数;不会从句子中间截断')}
								{kv('裁剪原则', '必要内容(你的问题 / 系统提示 / 案例主体)优先保全 → 给资料 / 检索留出尾仓 → 各技法层按水位公平分摊;某层低于保底字数时宁可整层记为「未纳入」也不静默截半')}</div>
							<div style={card}><div style={ct}>挂载预览卡(逐层查看)</div>
								{kv('展开', '抽屉内每一「前提层」是一个可折叠面板,展开即看该层将发送的快照正文,做到所见即所发')}
								{kv('层标记', '每层标注类型(案例 / 技法 / 资料等)、状态,若技法层改过参数标「已自定义」,超限时同步显示上面的红 / 橙裁剪标签')}
								{kv('技法层齿轮', '技法层右侧「设置」按钮:就地打开该技法的挂载设置,调「纳入内容」分段勾选与排盘 / 起卦选项,「已改」表示已偏离默认')}</div>
							<div style={card}><div style={ct}>组合 · 默认检索策略</div>
								{kv('作用', '在「模版」页的「组合」上可绑一档默认检索策略,决定套用该组合时资料库以何种方式进入上下文')}
								{kv('自动(推荐)', '按内容长度自适应:短则直接全文,长则改走向量检索取相关片段')}
								{kv('全文优先', '尽量把引用资料原文整体纳入(更完整,更占预算)')}
								{kv('检索优先', '只取与当前问题最相关的片段(更省预算,需已配嵌入 / 向量模型)')}</div>
						</div>
					</TabPane>
