				...
				<Tabs defaultActiveKey="overview" size="small">
					...
					<TabPane tab="时间与参数" key="input">
						<div style={body}>
							...
							<div style={card}><div style={ct}>盘面参数</div>
								...
								{kv('星座口径', '天文星座 / 涵义星座 两种画法;只对南纬盘生效,北半球两种画法相同(北纬时下拉置灰)')}
								{kv('中心体切换', '把天球的中心从「地心(默认)」换成「日心 / 月心」或任一行星心(水星心…冥王心)。换心后 3D 球体与轨道按新中心重算并有过渡动画（右栏「信息 / 相位 / 行星 / 希腊点」与 AI 导出仍为地心盘）;非地心时改时间 / 地点会以同一中心原地刷新;计算失败会自动回落到地心并提示')}
					...
					<TabPane tab="显示设置" key="display">
						<div style={body}>
							...
							<div style={card}><div style={ct}>开关(勾选即生效;缺省:有云地球、隐藏地球附近星体 开,其余关)</div>
								...
								{kv('使用虚拟28宿 / 隐藏28宿距星', '前者改用虚拟宿位:以立春时瑶光(北斗第七星)赤经起算、按古宿度不等宽排布的 28 个宿点,代替按真实距星摆的宿点;后者隐去二十八宿的距星标记(两组切换时同样遵守)')}
							...
							<div style={card}><div style={ct}>参数(滑块 / 下拉,拖动即时反映到画面)</div>
								...
								{kv('摄像机天球经度 / 纬度', '经度 0–360、纬度 −90–90,直接以数值定位观察方位(与视角预设等价的手动版);经纬按黄道坐标计,已计入盘面随上升点的整组旋转,任意上升点下「经度 0」都正对春分点')}
								{kv('太阳光强度 / 环境光强度', '场内星点与线条不受光,这两档的可见效果是:前者 0–6.5 调太阳光晕的浓淡(6.5 = 全亮),后者 0–0.3 调恒星 / 距星星点的亮度(0.3 = 全亮);建盘与刷新后按已存的值应用')}
								{kv('恒星距离行星圈', '0–500,把恒星层放到离中心多远的球壳上')}
								{kv('恒星半径', '0.5–8,恒星标记的大小')}</div>
							<div style={card}><div style={ct}>颜色(取色器,随选随变)</div>
								{kv('可调项', '星盘背景 / 太阳光颜色(太阳光晕色调) / 环境光颜色(星点色调) / 天球线条颜色 / 文本颜色')}
