				...
				<Tabs defaultActiveKey="overview" size="small">
					<TabPane tab="总览" key="overview">
						<div style={body}>
							...
							<ul style={ul}>
								...
								<li style={li}>右栏末段<b>「演法」</b>另起 盘 / 起禽 / 择日 / 占卜 / 投胎 / 定局 / 拆字 / 宿曜 八个子页签,自带日期 / 时辰 / 流派选择,与主命盘独立(圆形演禽盘、排四禽、择吉日、占一事、投胎度数、完整定局查表、拆字演禽、宿曜道三九)。</li>
							</ul>
					...
					<TabPane tab="右栏看什么" key="right">
						<div style={body}>
							...
							<div style={card}><div style={ct}>演法(八子页签)</div>
								{kv('盘', '圆形演禽盘:当日盘子时正禽置子、十二地支顺排时禽;当前时辰高亮、我禽绿边 / 彼禽红边;附三传流转弧')}
								{kv('起禽 / 择日 / 占卜 / 投胎', '四禽起例与定局、值日吉凶歌 + 黄黑道建除 + 克应应兆择吉、仿大六壬三传四课占一事(锁泊 / 分类占 / 吞啖)、投胎度数十二禽兽 + 十二宫字位')}
								{kv('定局', '完整查表:日禽 60×7、月禽 7×12、年禽三元 1864–2043(当前盘高亮)')}
								{kv('拆字 / 宿曜', '独立子系统:拆字演禽(报数 + 笔画配七政 + 八门)、宿曜道三九(27 宿去牛 + 业胎 + 相性)')}</div>
							<div style={h}>演法 · 流派设置(演法区顶部)</div>
							<div style={card}><div style={ct}>流派预设(七派 + 自定义)</div>
								{kv('主流六派', '江西派宗本 / 江西派集大成 / 广东派 / 江西派翻禽倒将 / 现代凤凰占课 / 翻禽倒将传本')}
								{kv('变体一派', '汪绂双池派(禽星五行重配·亢火牛木)')}
								{kv('主要分野', '江西一系重活曜番禽、翻禽倒将判吉凶;广东一系重三传锁泊;凤凰系时禽=我、完整三传四课不立命宫')}</div>
							<div style={card}><div style={ct}>六档互锁开关(占卜 / 五行体系分歧)</div>
								{kv('我 / 彼归属', '翻禽=我 / 时禽=我 / 到将=我 —— 定体用双方,凤凰系与江西宗本恰相反')}
								{kv('时禽旬头位移', '加位移 / 不加(算例口径)')}
								{kv('月禽口诀', 'A 版(主流) / B 版(别系)')}
								{kv('活曜传本', '不立活曜 / 番禽系(土→翼) / 翻禽系(土→箕)')}
								{kv('占卜提示侧重', '三传+翻禽并用 / 重三传锁泊(粤) / 重翻禽倒将(赣)。只改占卜页的断法提示句与默认展开的子页签,不改任何判语与吉凶')}
								{kv('二十八禽五行', '常规七政 / 汪绂重配(亢火牛木坐实,余宿待校回退七政);改此项占断生克随之变')}</div>
							<p style={{ ...p, color: MUTED }}>右栏当前所见即 AI 分析挂载与导出内容,所见即所得。部分内容(克应后十宿、暗金伏断逐宿表、王肯堂积年 / 钟林晃分禽算法、鉴形赋中段、三世相书表)古本源头残缺,已诚实标「待校」,不臆造。</p>
						</div>
					</TabPane>
