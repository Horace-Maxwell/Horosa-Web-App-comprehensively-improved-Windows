...
class AstroGivenYear extends Component{
	...
	render(){
		...
		const chartTab = this.state.chartTab || 'doublechart';
			...
			<div className="horosa-givenyear-page">
				<Row className="horosa-givenyear-layout" gutter={6}>
					<Col className="horosa-givenyear-chart-panel" span={17}>
						...
						{/* horosa_freeze_subtabs_v1:三张盘此前全部常驻重渲。改受控 + FreezeSubTab:
						    只有前台那张参与重渲;切回时拿本轮最新 children 立即渲一帧,不卸载、不重发请求、
						    不丢滚动位置。截图只取激活面板,AI 快照走数据不走 DOM,导出/截图零影响。 */}
						<Tabs
