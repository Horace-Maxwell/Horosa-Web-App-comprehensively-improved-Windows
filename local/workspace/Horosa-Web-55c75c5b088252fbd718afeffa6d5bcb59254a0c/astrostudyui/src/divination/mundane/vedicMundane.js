...
import { chartAtMoment, fetchMundaneEvents } from './momentPipeline';
...
async function solveVedicSolarIngress(eventKey, year, fieldsLike){
	const target = EVENT_TARGET[eventKey];
	if(target == null){ return null; }
	const f = { ...(fieldsLike || {}), zodiacal: 1, siderealAyanamsa: 'lahiri', hsys: 0, tradition: 0 };
	let moment = `${year}-${APPROX_DAY[eventKey]} 12:00:00`;
	for(let i = 0; i < 4; i++){
		const chart = await chartAtMoment(moment, f);
		if(!chart){ return null; }
		const objs = (chart.chart && chart.chart.objects) || [];
		const sun = objs.find((o) => o.id === 'Sun');
		if(!sun || sun.lon == null){ return null; }
		let diff = fwd(sun.lon, target);
		if(diff > 180){ diff -= 360; }   // 🔴 首步也须取最短带符号弧:曾加 i>0 守卫 →
		// 种子晚于真实入境时 diff≈360⁻ 被当成「向前一整年」,收敛到次年的入境盘
		// (西半球时区下 20 世纪多数年份全错);与回归求根「禁 clamp、双向收敛」同纪律。
		if(Math.abs(diff) < 0.002){ return { moment, chart }; }
		const ms = new Date(String(moment).replace(/-/g, '/')).getTime() + (diff / 0.98565) * 86400000;
		const d = new Date(ms);
		const pad = (n) => (n < 10 ? '0' + n : '' + n);
		moment = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`;
	}
	return { moment, chart: null };
}

export { solveVedicSolarIngress };
...
export async function computeNavanayaka(year, fieldsLike){
	const offices = [];
	let meshaMoment = null;
	for(const off of NAVANAYAKA_OFFICES){
		if(off.event === 'lunar_new_year'){ offices.push({ ...off, lord: null, moment: null }); continue; }
		const r = await solveVedicSolarIngress(off.event, year + (off.yearOffset || 0), fieldsLike);
		const moment = r ? r.moment : null;
		if(off.event === 'ingress_0'){ meshaMoment = moment; }
		offices.push({ ...off, moment, lord: moment ? varaLordOf(moment) : null });
	}
	// 王:梅沙入境前 30 日窗内最近新月。
	if(meshaMoment){
		try{
			const end = meshaMoment.slice(0, 10);
			const startMs = new Date(end.replace(/-/g, '/')).getTime() - 35 * 86400000;
			const sd = new Date(startMs);
			const pad = (n) => (n < 10 ? '0' + n : '' + n);
			const start = `${sd.getFullYear()}-${pad(sd.getMonth() + 1)}-${pad(sd.getDate())}`;
			const ev = await fetchMundaneEvents({ startDate: start, endDate: end, zone: fieldsLike && fieldsLike.zone, lat: fieldsLike && fieldsLike.lat, lon: fieldsLike && fieldsLike.lon, gpsLat: fieldsLike && fieldsLike.gpsLat, gpsLon: fieldsLike && fieldsLike.gpsLon, kinds: ['lunations'] });
			// lunations 归一化字段=phase('new'/'full')+localTime(见 momentPipeline);取窗内最后一次新月。
			const nms = ((ev && ev.lunations) || []).filter((l) => String(l.phase || '').toLowerCase().indexOf('new') >= 0);
			const last = nms.length ? nms[nms.length - 1] : null;
			const raja = offices.find((o) => o.key === 'raja');
			if(raja && last && last.localTime){
				raja.moment = last.localTime;
				raja.lord = varaLordOf(raja.moment);
			}
		}catch(e){ /* 王位留空,UI 提示 */ }
	}
	// 兼职与冲突。
	const byLord = {};
	offices.forEach((o) => { if(o.lord){ (byLord[o.lord] = byLord[o.lord] || []).push(o.cn); } });
	const raja = offices.find((o) => o.key === 'raja');
	const mantri = offices.find((o) => o.key === 'mantri');
	const rajaMantriClash = !!(raja && mantri && raja.lord && raja.lord === mantri.lord);
	// 口诀。
	const argha = offices.find((o) => o.key === 'arghadhipati');
	const rasa = offices.find((o) => o.key === 'rasadhipati');
	const readings = [];
	if(argha && argha.lord){
		if(argha.lord === 'sun'){ readings.push('价主为日 → 物价趋跌'); }
		else if(argha.lord === 'moon' || argha.lord === 'mercury'){ readings.push('价主为' + PLANET_CN_V[argha.lord] + ' → 物价趋涨'); }
	}
	if(rasa && rasa.lord){
		if(rasa.lord === 'moon'){ readings.push('汁主为月 → 油糖类趋涨'); }
		else if(rasa.lord === 'sun'){ readings.push('汁主为日 → 油糖类趋跌'); }
	}
	if(rajaMantriClash){ readings.push('⚠ 同一行星兼王与相 → 全年统治者受困、火盗之灾'); }
	return { offices, byLord, rajaMantriClash, readings };
}
...
export default {
	VARA_LORDS, NAVANAYAKA_OFFICES, NAVANAYAKA_NOTES, computeNavanayaka, varaLordOf,
	VIMSHOTTARI_SEQ, VIMSHOTTARI_MUNDANE_MEANINGS, vimshottariFromMoon, kpSubLordAt, KP_NOTES,
	NAKSHATRA_27, NAK_KEYPOINTS, GARBHA_CONST, garbhaDeliveryDate, buildSaptaNadi, SAPTA_NADI_COLS,
	ARGHA_RULES, TRANSIT_RULES, ECLIPSE_VEDIC_RULES, KURMA_MODERN, munthaSign, solveVedicSolarIngress,
};
