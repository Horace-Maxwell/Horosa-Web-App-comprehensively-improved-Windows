...
import { chartAtMoment } from './momentPipeline';
...
export async function solveSiderealIngress(typeKey, year, fieldsLike, seedMoment){
	const t = SOLUNAR_TYPES.find((x) => x.key === typeKey);
	if(!t){ return null; }
	const f = { ...(fieldsLike || {}), zodiacal: 1, siderealAyanamsa: 'fagan_bradley', hsys: 10, tradition: 0 };
	let moment = seedMoment || (t.body === 'sun' ? `${year}-${t.approx} 12:00:00` : `${year}-01-01 12:00:00`);
	// 近种子=显式 seedMoment 或太阳 approx(±2 日内):首步即取最短带符号弧 —— 种子晚于真入境时
	// diff≈360⁻ 曾被当「向前一整年」收敛到次年盘(西半球 20 世纪多数年份全错)。
	// 🔴 月盘无 approx、种子 1/1 是「远种子」:首步必须只向前,否则约半数命中上一年 12 月的回归
	//   (月速 13°/日,1/1 距目标常 >180°);其后各步已贴近真根,恢复双向微调。
	const nearSeed = !!seedMoment || t.body === 'sun';
	const speed = MEAN_SPEED[t.body];
	let chart = null;
	for(let i = 0; i < (t.body === 'sun' ? 4 : 6); i++){
		chart = await chartAtMoment(moment, f);
		if(!chart){ return null; }
		const lon = lonOf(chart, t.body);
		if(lon == null){ return null; }
		let diff = fwd(lon, t.target);            // 前向弧(入境=向前到达目标)
		if(diff > 180 && (nearSeed || i > 0)){ diff -= 360; }
		if(Math.abs(diff) < 0.002){ break; }      // ~7″ 收敛(太阳≈3 分钟)
		moment = shiftMoment(moment, diff / speed);
	}
	return { moment, chart };
}
...
export default { SOLUNAR_TYPES, SOLUNAR_WEIGHTS, ANGULARITY_ORBS, PLANET_ANGULAR_OMEN, computeAngularity, isDormantChart, rulerDeathSignature, solveSiderealIngress, describeSolunar, mundoPositionOf };
