...
export function collectTestimoniesV2(ctx){
	...
	if(facts && querentKey && quesitedKey && querentKey !== quesitedKey){
		const { mutualReceptionBetween, receives } = require('../engine/reception');
	...
	if(facts && querentKey && quesitedKey){
		const { antisciaPairsOf } = require('../engine/resultShapes');
		const { chartIdOfKey } = require('../engine/utils');
