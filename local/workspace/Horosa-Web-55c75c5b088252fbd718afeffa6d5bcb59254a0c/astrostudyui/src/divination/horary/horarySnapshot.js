...
import { PLANETARY_HOURS } from '../data/planetaryHours';
...
export function buildHorarySnapshot(j, chart, opts3){
	...
	if(j.facts && j.facts.planets){
		const { buildAntisciaTable } = require('./antisciaTable');
	...
	// [断法要点] 题型判断重点+典型吉凶徵。
	{
		const { getQuestionGuide } = require('./questionGuide');
	...
	// [自然象征] 该事项自然征象星完整词条。
	if(sig.natural){
		const { NATURAL_SIGNIFICATORS } = require('../data/naturalSignificators');
