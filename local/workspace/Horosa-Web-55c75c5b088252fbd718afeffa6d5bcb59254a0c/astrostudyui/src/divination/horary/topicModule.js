...
function buildMarriageTopic(facts){
	...
	const { aspectBetween } = require('../engine/aspectsEngine');
	...
	if(l1 && l7 && l1 !== l7){
		const { mutualReceptionBetween } = require('../engine/reception');
...
function buildLostAnimal(facts){
	...
	const { DIR_BY_ELEMENT } = require('../data/directions');
...
function buildLostObject(facts, opts){
	...
	if(asc != null && sun != null && moon != null){
		...
		const { signOfLon } = require('../data/signs');
		const pofSign = SIGNS[signOfLon(pofLon)];
		const { lotDispositor } = require('../data/lots');
