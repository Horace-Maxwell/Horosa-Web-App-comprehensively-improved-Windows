...
// 供 aiExportDocModel(导出 IR 解析) / docxCommon(真 Word 表) / reportExport(报告 md→docx) 共用,
// 三处此前各自持有或即将持有同款正则——集中一处防口径漂移。
// 契约与 reportExport.js 历史实现逐字一致(v2 底座抽取,行为零变化)。
...
// 分隔行 → 每列对齐('left'|'center'|'right'),与 reportExport makeDocxParagraphsFromMarkdown 内联逻辑同款。
