...
class PerPredict:
    ...
    def _astroappEqCoords(self, lon, lat, obliquity):
        eq = utils.sweCotrans(float(lon), float(lat), 1.0, -float(obliquity))
    ...
    def _pdChartEqToEcl(self, ra, decl, obliquity):
        ecl = utils.sweCotrans(float(ra), float(decl), 1.0, float(obliquity))
    ...
    def _pdChartBuildAnglesAndHouses(self, chart, arc, obliquity):
        ...
        asc_lat = utils.sweCotrans(float(dir_ascmc[4]), lat, 1.0, float(obliquity))[1]
