# -*- coding: utf-8 -*-
# [Z8] 印度择日·征象扫描端点(算法本体在 astrostudy/india_election_scan.py,本文件只做薄壳)。
...
from astrostudy import india_election_scan
from websrv.helper import enable_crossdomain


class IndiaElectionScanSrv:
    exposed = True
    ...
    def scan(self, **params):
        ...
        try:
            ...
            result = india_election_scan.scan(payload)
        except Exception:
            traceback.print_exc()
    ...
    def conditiontypes(self, **params):
        ...
        return self._json({'ResultCode': 0, 'Result': {
            'types': sorted(india_election_scan.CONDITION_TYPES.keys()),
            'groups': list(india_election_scan.GROUP_TYPES),
        }})
    ...
    def explain(self, **params):
        ...
        try:
            payload = self._payload(params)
            result = india_election_scan.explain_at(payload)
        except Exception:
            traceback.print_exc()
