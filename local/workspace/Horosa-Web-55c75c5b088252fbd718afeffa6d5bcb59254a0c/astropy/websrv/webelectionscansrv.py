# -*- coding: utf-8 -*-
# 天星择日·征象搜索端点(Calculate 侧;算法本体在 astrostudy/election_scan.py,本文件只做薄壳)。
...
class ElectionScanSrv:
    ...
    def scan(self, **params):
        ...
        try:
            ...
            result = election_scan.scan(payload)
        except Exception:
            traceback.print_exc()
    ...
    def conditiontypes(self, **params):
        ...
        return self._json({'ResultCode': 0, 'Result': {
            'types': sorted(election_scan.CONDITION_TYPES.keys()),
            'groups': list(election_scan.GROUP_TYPES),
        }})
    ...
    def explain(self, **params):
        ...
        try:
            payload = self._payload(params)
            result = election_scan.explain(payload)
        except Exception:
            traceback.print_exc()
