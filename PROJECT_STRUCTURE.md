...
- 目录 `Horosa-Web+App (Mac)/` 已删除，当前运行仅依赖本目录现存文件与 `runtime/`。
...
- 逐行语义拆解文件：
  ...
  - 每行结构：`lineNo / tag / intent / sourceText`，总行数 4726，与源文档逐行。
- 审计汇总文件：
...
- 三式合一中的六壬展示已收口为“纯命中输出”：
  ...
  - `大格/小局` 保持算法命中项展示，取消“去页查全文”的引导。
- 三式合一 AI 快照同步改为命中式：
  - 删除 `六壬-` 段与概览中的全文扩展，保留可计算结果与命中条目。
