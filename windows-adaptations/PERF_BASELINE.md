...
- version: 3.10.0
- build: release(v3.10.0 **择日十技法大版本轮**:择日页 2→10 技法[黄历/八字/太乙/紫微/六壬/
  三式/七政/印度全量落地,约 300 条件类,判定与主盘同源零第二实现]+ 真太阳时 EoT 升 NOAA/Meeus +
  AI 流式超时三层语义[修 issue #77]+ 紫微挂载设置隔离[修 issue #76]+ 盘面文字填色渲染;
  **启动路径核过**:壳 0 文件;Python 新增 2 个扫描服务已按 #89 显式入 **POST_GATE**(门前预算表
  零扩,温启结构不变);Java 仅 AI 代理超时重构 + RuntimeWire(均不在启动路径)。overlay 相交 27
  目标全部 regen + round-trip 27/27 全等;TaiYiMain #94 三连击由总账门实弹拦截并按 #100 收口。
- date: 2026-09-01(v3.10.0 同步轮;逐技法全表存档日 2026-07-22,本轮未重跑逐技法全表)
- commit: 见本轮发布 commit(v3.10.0 同步轮)
