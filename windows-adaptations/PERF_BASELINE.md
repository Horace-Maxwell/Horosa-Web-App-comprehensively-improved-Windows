...
- version: 3.6.1
- build: release(v3.6.1 修复打磨轮:失效开关四修/窄栏截断治理/数算三修+AI 模型代际归一/
  启动观感统一;**零 Python 改动、零新依赖** ⇒ 计算路径与启动路径均无结构性变更;
  核心 10 键验收当轮重跑[见「v3.6.1 键石复测」节],全 25 键表沿 07-22 口径存档,
  台架对 3.6.x 新 UI 的全量适配仍为跟进项)
- date: 2026-08-01(键石复测与启动面实测日;逐技法全表存档日 2026-07-22)
- commit: 见本轮发布 commit(v3.6.1 同步轮)
