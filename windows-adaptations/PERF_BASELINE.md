...
- version: 3.9.3
- build: release(v3.9.3 **占星古典设置大扩充轮**:星盘设置约四十项古典口径档位全链透传
  [显示/计算/AI 挂载/AI 导出/存档/备份单源]+ 设置面板全宽行式改版 + 右栏各表随设置即时联动
  + 塔罗升「卜」一级导航[卜·其他聚合页摘除,存量 tab 白名单兜底回落 suzhan]+
  **挂载存档竞态根治**[8 文件 13 处「快照与页面过同一校正函数、异步落地补拍快照」,上游自动闸随行]
  + 八字大运同源;**启动路径零改动**[Electron 壳 0 文件、Python 就绪链 0 改动、Java 四控制器
  只加古典参数透传 + RuntimeWire 版本号];性能面观察点:①占星族十余个 perfR9 切片组件与上游
  古典扩充相交,已逐文件合并并经编排 round-trip 53/53 逐字节自证;②异步补拍快照
  (saveGuolaoAISnapshot/saveAISnapshot 等)全部并入我方既有 markPanelReady 回调,不加渲染拍。
- date: 2026-08-20(v3.9.3 同步轮;逐技法全表存档日 2026-07-22,本轮未重跑逐技法全表)
- commit: 见本轮发布 commit(v3.9.3 同步轮)
